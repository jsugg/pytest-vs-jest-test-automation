...
1. **Test scenario 1**: Verify that the search engine [DuckDuckGo](duckduckgo.com) correctly finds and displays the [expected website](https://www.football-data.org/) when searching for `The dev-friendly football API`.
...
#### Reporting
Passing the `--html` flag followed by `=` and a `filename.html`, will create a test report file named _filename.html_.
